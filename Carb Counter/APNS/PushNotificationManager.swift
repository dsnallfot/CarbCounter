...
class PushNotificationManager {
    ...
    init() {
        ...
        self.user = UserDefaultsRepository.caregiverName
    ...
    func sendMealPushNotification(
        ...
        guard carbsValue != nil || proteinValue != nil || fatValue != nil || bolusAmountValue != nil else {
            completion(false, "No nutrient data provided. At least one of carbs, fat, protein or bolus must be greater than 0.")
            return
        }
    ...
    private func validateCredentials() -> [String]? {
        ...
        if !apnsKey.contains("-----BEGIN PRIVATE KEY-----") || !apnsKey.contains("-----END PRIVATE KEY-----") {
            ...
            if let keyData = extractKeyData(from: apnsKey) {
                print("APNS Key Data (Extracted): \(keyData.prefix(30))...") // Show only the first 30 characters for brevity
                if Data(base64Encoded: keyData) == nil {
                    errors.append("APNS Key contains invalid Base64 key data.")
                } else {
                    print("APNS Key is valid Base64.")
                }
            } else {
